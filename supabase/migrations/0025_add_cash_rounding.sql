alter table public.sales
  add column cash_rounding_amount bigint not null default 0
    check (cash_rounding_amount between -5 and 4);

alter table public.sales drop constraint sales_check1;
alter table public.sales
  add constraint sales_cash_payment_check check (
    (
      payment_method = 'cash'
      and cash_received is not null
      and change_amount is not null
      and cash_received >= total + cash_rounding_amount
      and change_amount = cash_received - (total + cash_rounding_amount)
    )
    or (
      payment_method <> 'cash'
      and cash_received is null
      and change_amount is null
      and cash_rounding_amount = 0
    )
  );
...
declare
  ...
  v_qty numeric(12,3);
  v_line_total bigint;
  v_cash_rounding bigint := 0;
  v_cash_payable bigint := 0;
  v_last_digit integer := 0;
...
begin
  ...
  if p_payment = 'cash' then
    v_last_digit := mod(v_total, 10);
    if v_last_digit between 1 and 5 then
      v_cash_rounding := -v_last_digit;
    elsif v_last_digit between 6 and 9 then
      v_cash_rounding := 10 - v_last_digit;
    end if;
    v_cash_payable := v_total + v_cash_rounding;
    if coalesce(p_cash_received, 0) < v_cash_payable then
      raise exception 'Efectivo insuficiente';
    end if;
  else
    v_cash_payable := v_total;
  end if;
  ...
  insert into public.sales(
    ...
    total, cash_received, change_amount, cash_rounding_amount, created_by,
    ...
    case when p_payment='cash' then p_cash_received-v_cash_payable end,
    v_cash_rounding, auth.uid(), v_fee_model, v_fee_percentage, v_fee_fixed,
    v_fee_net, v_fee_vat_rate, v_fee_tax, v_total-v_fee_net-v_fee_tax,
    v_settlement_days
