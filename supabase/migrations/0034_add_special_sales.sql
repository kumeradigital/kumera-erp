alter table public.sales
  add column sale_kind text not null default 'regular'
    check (sale_kind in ('regular', 'special_order')),
  add column scheduled_for date,
  add column customer_name text,
  add column sale_note text;

create index sales_special_orders_idx
  on public.sales (business_id, scheduled_for)
  where sale_kind = 'special_order';
