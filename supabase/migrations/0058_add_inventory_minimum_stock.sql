alter table public.ingredients
  add column inventory_minimum_quantity integer not null default 0
    check (inventory_minimum_quantity >= 0);
...
begin
  ...
  for v_item in select * from jsonb_array_elements(p_items) loop
    ...
    v_minimum_quantity := coalesce(nullif(v_item->>'minimum_quantity', '')::integer, 0);
    ...
    if v_minimum_quantity < 0 then
      raise exception 'El stock mínimo debe ser igual o mayor que cero';
    end if;
    ...
    if v_item_type = 'ingredient' then
      ...
      set inventory_quantity = v_quantity,
          inventory_minimum_quantity = v_minimum_quantity,
